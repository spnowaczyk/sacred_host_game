...
Map::Map() {
    ...
    sdlTexture_concrete = TextureManager::LoadTexture("../assets/concrete.png");
...
void Map::LoadMap(int tileArray [12][20]) {
    for (int row = 0; row < 12; ++row) {
        for (int col = 0; col < 20; ++col) {
            this->iA_tileMap[row][col] = tileArray[row][col];
        }
    }
}
...
void Map::RandomMap() {
    for (int row = 0; row < 12; ++row) {
        for (int col = 0; col < 20; ++col) {
            this->iA_tileMap[row][col] = rand() % 3; // generates random numbers between 0 and 2
        }
    }
}
